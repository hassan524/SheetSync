...
const GridBackground = ({ variant = "default", className = "" }: GridBackgroundProps) => {
  const patterns = {
    default: (
    ...
    cells: (
      <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="grid-cells" width="24" height="24" patternUnits="userSpaceOnUse">
            <rect
              x="0"
              y="0"
              width="10"
  ...
  return (
    <div className={`absolute inset-0 text-primary pointer-events-none ${className}`}>
      {patterns[variant]}
    </div>
  );
};
