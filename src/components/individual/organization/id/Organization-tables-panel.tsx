...
function NoMembersIcon() {
  return (
    ...
    >
      ...
      />
      <path
        d="M22 58c0-10 9-17 20-17s20 7 20 17"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        fill="currentColor"
    ...
    render: (s: Sheet) => (
      <div className="flex items-center gap-2">
        <Avatar className="h-6 w-6 shrink-0">
          {s.owner.avatar && (
            <AvatarImage src={s.owner.avatar} alt={s.owner.name} />
          )}
    ...
    render: (s: Sheet) => (
      <div>
        ...
        <p className="text-[11px] text-muted-foreground/50 mt-0.5">
          by{" "}
          {(s.lastModifiedBy ?? s.last_modified_by ?? "Unknown").split(" ")[0]}
        </p>
      </div>
    ),
  },
...
const memberColumns = [
        ...
        <div className="relative shrink-0">
          <Avatar className="h-7 w-7">
            {m.avatar?.startsWith("http") && (
              <AvatarImage src={m.avatar} alt={m.profiles.name} />
            )}
  ...
  },
  {
    ...
    render: (m: Member) => (
      <span
        className={`text-[11px] font-medium px-2 py-0.5 rounded-full capitalize whitespace-nowrap ${ROLE_STYLE[m.role ?? "viewer"]}`}
      >
        {m.role ?? "viewer"}
      </span>
    ),
  },
  {
    ...
    render: (m: Member) => (
      <div className="flex items-center gap-1.5">
        <span
          className={`h-1.5 w-1.5 rounded-full shrink-0 ${m.status === "online" ? "bg-green-500" : "bg-slate-300"}`}
        />
    ...
    <Tabs defaultValue="sheets">
      ...
      <TabsContent value="sheets" className="mt-0">
        <DataTable
          ...
          onRowClick={(s) =>
            router.push(
              `/sheet/${s.id}?org=true&template=${s.template_id ?? "blank"}`,
            )
          }
