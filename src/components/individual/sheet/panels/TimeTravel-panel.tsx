...
function ActionLabel({
  ...
  if (entry.kind === "snapshot") {
    return (
      <span className={`text-[11px] font-semibold ${d ? "text-gray-200" : "text-gray-800"}`}>
        Saved snapshot
      </span>
    );
  }
  ...
  if (action === "cell_edit") {
    const match = detail.match(/^Edited (\S+) \(([^)]+)\): "([^"]*)" → "([^"]*)"/);
    if (match) {
      ...
      return (
        <div>
          <span className={`text-[10.5px] ${d ? "text-gray-300" : "text-gray-700"}`}>
            Edited{" "}
            <span className={`font-mono font-bold text-[10px] px-1 py-0.5 rounded ${d ? "bg-gray-800 text-primary" : "bg-primary/8 text-primary"}`}>
              {cell}
            </span>
            <span className={`ml-1 text-[10px] ${d ? "text-gray-500" : "text-gray-400"}`}>
              {col}
            </span>
          </span>
          {(oldV || newV) && (
            <div className="flex items-center gap-1 mt-0.5">
              <span className={`text-[10px] line-through ${d ? "text-gray-600" : "text-gray-400"}`}>
                {oldV || "empty"}
              </span>
              <span className={`text-[9px] ${d ? "text-gray-600" : "text-gray-400"}`}>→</span>
              <span className={`text-[10px] font-medium ${d ? "text-emerald-400" : "text-emerald-600"}`}>
                {newV || "empty"}
              </span>
            </div>
          )}
  ...
  if (action === "column_rename") {
    ...
    if (match) {
      ...
      return (
        <span className={`text-[10.5px] ${d ? "text-gray-300" : "text-gray-700"}`}>
          Renamed col{" "}
          <span className={`line-through ${d ? "text-gray-600" : "text-gray-400"}`}>{oldName}</span>
          {" → "}
...
function ForkDialog({
  snapshot, isDark, onConfirm, onCancel,
}: {
  snapshot: SheetSnapshot; isDark: boolean;
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className={`w-full max-w-[260px] rounded-2xl border shadow-2xl p-4 space-y-3
        ${d ? "bg-gray-950 border-gray-700" : "bg-white border-gray-200"}`}>

        <div className="flex items-center gap-2.5">
          <div className={`h-8 w-8 rounded-xl flex items-center justify-center shrink-0 ${d ? "bg-emerald-900/40" : "bg-emerald-50"}`}>
            <GitBranch className="h-3.5 w-3.5 text-emerald-500" />
          </div>
          <div>
            <p className={`text-[12px] font-bold ${d ? "text-gray-100" : "text-gray-900"}`}>Fork snapshot</p>
            <p className={`text-[10px] ${d ? "text-gray-500" : "text-gray-400"}`}>Creates a new independent sheet</p>
          </div>
        </div>

        <div className={`rounded-lg px-2.5 py-2 text-[10px] ${d ? "bg-gray-900 border border-gray-800" : "bg-gray-50 border border-gray-100"}`}>
          <p className={`font-semibold truncate ${d ? "text-gray-300" : "text-gray-700"}`}>{snapshot.label ?? "Auto-save"}</p>
          <p className={`mt-0.5 ${d ? "text-gray-600" : "text-gray-400"}`}>
            {fmtTime(snapshot.created_at)} · {snapshot.rows_data.length}r · {snapshot.columns_data.length}c
          </p>
        </div>

        <div className="space-y-1">
          <label className={`text-[10px] font-semibold uppercase tracking-wider ${d ? "text-gray-500" : "text-gray-400"}`}>
            New sheet name
          </label>
...
function SnapshotCard({
  snap, isDark, onBranch, onRename, onDelete, alreadyForked,
}: {
  snap: SheetSnapshot; isDark: boolean;
  onBranch: () => void; onRename: (l: string) => void; onDelete: () => void;
  ...
  const commit = () => { if (label.trim()) onRename(label.trim()); setEditing(false); };
    ...
    <div className={`rounded-xl border p-2.5 transition-all
      ...
      {editing ? (
        <div className="flex items-center gap-1 mb-2">
          ...
          />
          <button onClick={commit} className={`p-1 rounded ${d ? "hover:bg-gray-800" : "hover:bg-gray-100"}`}>
            <Check className="h-3 w-3 text-emerald-500" />
          </button>
          <button onClick={() => setEditing(false)} className={`p-1 rounded ${d ? "hover:bg-gray-800" : "hover:bg-gray-100"}`}>
        ...
        <div className="flex items-start justify-between gap-1.5 mb-2">
          <div className="flex-1 min-w-0">
            <p className={`text-[11px] font-semibold truncate ${d ? "text-gray-200" : "text-gray-800"}`}>
              {snap.label ?? "Auto-save"}
            </p>
            <p className={`text-[9.5px] mt-0.5 ${d ? "text-gray-600" : "text-gray-400"}`}>
              {fmtTime(snap.created_at)} · {snap.rows_data.length}r · {snap.columns_data.length}c
              {snap.creator_name ? ` · by ${snap.creator_name}` : ""}
            </p>
          </div>
          <div className="flex items-center gap-0.5 shrink-0">
            <button onClick={() => { setEditing(true); setLabel(snap.label ?? ""); }}
...
function SaveBar({ onSave, isDark }: { onSave: (l: string) => void; isDark: boolean }) {
  ...
  return (
    <div className={`rounded-xl border p-2.5 space-y-2 ${d ? "border-primary/20 bg-primary/5" : "border-primary/15 bg-primary/5"}`}>
      <p className="text-[10px] font-semibold uppercase tracking-widest text-primary">Name this snapshot</p>
      <input autoFocus value={label} onChange={(e) => setLabel(e.target.value)}
        onKeyDown={(e) => {
          if (e.key === "Enter") { onSave(label.trim() || `Snapshot ${new Date().toLocaleTimeString()}`); setLabel(""); setOpen(false); }
      ...
      />
      <div className="flex gap-2">
        <button onClick={() => { onSave(label.trim() || `Snapshot ${new Date().toLocaleTimeString()}`); setLabel(""); setOpen(false); }}
...
export default function TimeTravelPanel({
  state, actions, isDark, orgMembers = [], onClose,
}: TimeTravelPanelProps) {
  const {
    snapshots, forkedSnapshotIds, timeline, playIndex, isPlaying,
    previewRows, isLoadingSnapshots, pendingForkSnapshot,
  } = state;
  ...
  const onDividerMove = useCallback((e: React.PointerEvent) => {
    ...
    const next = Math.max(MIN_TOP_HEIGHT, Math.min(panelH - MIN_BOTTOM_HEIGHT - 20, dragRef.current.startH + dy));
    setTopHeight(next);
  }, []);

  const onDividerUp = useCallback(() => { dragRef.current = null; }, []);

  const currentEntry = timeline[playIndex] ?? null;

  useEffect(() => {
    if (tab !== "timeline") return;
    const el = timelineRef.current?.querySelector(`[data-index="${playIndex}"]`);
  ...
  return (
    <div ref={panelRef} className="flex flex-col h-full relative overflow-hidden">

      {/* Fork dialog */}
      {pendingForkSnapshot && (
        <ForkDialog snapshot={pendingForkSnapshot} isDark={d}
          onConfirm={handleConfirmBranch} onCancel={actions.cancelBranch} />
      )}

      {/* Preview banner */}
      {previewRows && (
        <div className={`flex items-center gap-2 px-3 py-1.5 shrink-0 border-b
          ${d ? "border-gray-800 bg-amber-900/20" : "border-amber-100 bg-amber-50"}`}>
          <Eye className={`h-3 w-3 shrink-0 ${d ? "text-amber-400" : "text-amber-600"}`} />
          <p className={`text-[10.5px] font-medium flex-1 ${d ? "text-amber-300" : "text-amber-700"}`}>
            Previewing — grid is read-only
          </p>
          <button onClick={actions.exitPreview}
      ...
      <div className="shrink-0 overflow-hidden flex flex-col" style={{ height: topHeight }}>

        {/* Stats strip */}
        <div className={`flex items-center gap-3 px-3 py-2 shrink-0 border-b
          ${d ? "border-gray-800 bg-gray-900/30" : "border-gray-100 bg-gray-50/60"}`}>
          <span className={`flex items-center gap-1 text-[10.5px] font-medium ${d ? "text-gray-500" : "text-gray-400"}`}>
            <Clock className="h-3 w-3" />{timeline.length} events
          </span>
          <span className={`flex items-center gap-1 text-[10.5px] font-medium ${d ? "text-gray-500" : "text-gray-400"}`}>
            <Camera className="h-3 w-3" />{snapshots.length} snapshots
          </span>
        </div>
        ...
        <div className="flex-1 overflow-hidden px-3 pt-2.5 pb-1 flex flex-col gap-2.5 min-h-0">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <span className={`text-[10px] font-mono ${d ? "text-gray-600" : "text-gray-400"}`}>
                {playIndex + 1} / {Math.max(1, timeline.length)}
              </span>
              {currentEntry && (
                <span className={`text-[10px] truncate max-w-[140px] text-right ${d ? "text-gray-600" : "text-gray-400"}`}>
                  {((currentEntry as any).detail ?? (currentEntry as any).label ?? "").slice(0, 28)}
                </span>
              )}
            </div>
            ...
            >
              <div className="absolute left-0 top-0 h-full rounded-full bg-primary transition-all duration-150"
                style={{ width: timeline.length > 1 ? `${(playIndex / (timeline.length - 1)) * 100}%` : "0%" }} />
              <div className="absolute top-1/2 -translate-y-1/2 h-3.5 w-3.5 rounded-full bg-primary border-2 border-white shadow transition-all"
                style={{ left: timeline.length > 1 ? `calc(${(playIndex / (timeline.length - 1)) * 100}% - 7px)` : "-7px" }} />
              {timeline.map((entry, i) => {
                if (entry.kind !== "snapshot") return null;
                const pct = timeline.length > 1 ? (i / (timeline.length - 1)) * 100 : 0;
                return (
                  <div key={entry.id}
                    ...
                    onClick={(e) => { e.stopPropagation(); actions.seekTo(i); }}
                  />
                );
              })}
            </div>
          ...
          <div className="flex items-center gap-1.5">
            <button onClick={() => actions.seekTo(playIndex - 1)} disabled={playIndex === 0}
              ...
              <SkipBack className={`h-3.5 w-3.5 ${d ? "text-gray-400" : "text-gray-500"}`} />
            </button>

            <button onClick={isPlaying ? actions.pause : actions.play} disabled={timeline.length === 0}
            ...
            <button onClick={() => actions.seekTo(playIndex + 1)} disabled={playIndex >= timeline.length - 1}
              className={`h-7 w-7 rounded-lg flex items-center justify-center transition-all disabled:opacity-30 border
                ${d ? "border-gray-700 hover:bg-gray-800" : "border-gray-200 hover:bg-gray-50"}`}>
              <SkipForward className={`h-3.5 w-3.5 ${d ? "text-gray-400" : "text-gray-500"}`} />
            </button>

            <div className={`flex items-center gap-0.5 rounded-lg p-0.5 ${d ? "bg-gray-800" : "bg-gray-100"}`}>
              {([0.5, 1, 2] as const).map((s) => (
                <button key={s} onClick={() => actions.setSpeed(s)}
                  className={`h-6 px-2 rounded-md text-[10px] font-bold transition-all
                    ${state.speed === s
                      ? d ? "bg-gray-700 text-gray-200" : "bg-white text-gray-800 shadow-sm"
          ...
          {currentEntry?.kind === "snapshot" && (
            <button onClick={() => actions.branchFromSnapshot(currentEntry as SheetSnapshot)}
              ...
              {forkedSnapshotIds.has(currentEntry.id) ? "Fork again from here" : "Fork from this point"}
            </button>
          )}
        </div>
      </div>
      ...
      <div
        ...
        style={{ height: 18, background: d ? "#0f1117" : "#f9fafb", touchAction: "none" }}
      ...
      >
        <GripHorizontal className={`h-3.5 w-3.5 transition-colors ${d ? "text-gray-700 group-hover:text-primary/50" : "text-gray-300 group-hover:text-primary/40"}`} />
        <span className={`absolute right-2.5 text-[9px] font-medium opacity-0 group-hover:opacity-100 transition-opacity ${d ? "text-gray-600" : "text-gray-400"}`}>
          drag to resize
        </span>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-h-0">
        ...
        <div className={`flex items-center gap-1 px-3 h-9 shrink-0 border-b ${d ? "border-gray-800" : "border-gray-100"}`}>
            ...
            <button key={t} onClick={() => setTab(t)}
              className={`flex items-center gap-1.5 h-6 px-3 rounded-lg text-[11px] font-semibold capitalize transition-all
                ${tab === t
                  ...
                  : d ? "text-gray-500 hover:text-gray-300 hover:bg-gray-800/50" : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"}`}>
              {t === "timeline" ? <Clock className="h-3 w-3" /> : <Camera className="h-3 w-3" />}
            ...
            <div ref={timelineRef}>
              {timeline.length === 0 && (
                <div className="flex flex-col items-center justify-center py-10 px-4 text-center gap-2">
                  <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${d ? "bg-gray-800" : "bg-gray-100"}`}>
                    <Clock className={`h-5 w-5 ${d ? "text-gray-600" : "text-gray-400"}`} />
                  </div>
                  <p className={`text-[11px] font-semibold ${d ? "text-gray-400" : "text-gray-600"}`}>No activity yet</p>
                  <p className={`text-[10px] ${d ? "text-gray-600" : "text-gray-400"}`}>Events appear as you edit</p>
                </div>
              )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 pr-10">
                      {/* Name + time */}
                      <p className={`text-[10px] font-semibold mb-0.5 truncate ${d ? "text-gray-400" : "text-gray-500"}`}>
                        {userName}
                        {timeStr && (
                          <span className={`ml-1.5 font-normal ${d ? "text-gray-600" : "text-gray-400"}`}>
                            · {timeStr}
                          </span>
                        )}
                      </p>
                    ...
                    {isSnap && (
                      <button
                        onClick={(e) => { e.stopPropagation(); actions.branchFromSnapshot(entry as SheetSnapshot); }}
            ...
            <div className="p-3 space-y-2.5 pb-6">
              ...
              {!isLoadingSnapshots && snapshots.length === 0 && (
                <div className="flex flex-col items-center justify-center py-10 text-center gap-2">
                  <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${d ? "bg-gray-800" : "bg-gray-100"}`}>
                    <Camera className={`h-5 w-5 ${d ? "text-gray-600" : "text-gray-400"}`} />
                  </div>
                  <p className={`text-[11px] font-semibold ${d ? "text-gray-400" : "text-gray-600"}`}>No snapshots yet</p>
                  <p className={`text-[10px] leading-relaxed max-w-[180px] ${d ? "text-gray-600" : "text-gray-400"}`}>
              ...
              {snapshots.map((snap) => (
                <SnapshotCard
                  key={snap.id} snap={snap} isDark={d}
