    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
                <div className="p-3">
                    {history.length === 0 && (
                        <div className={`text-center py-10 text-[12px] ${isDark ? "text-gray-600" : "text-gray-400"}`}>
                            No changes yet
                        </div>
                    )}
                    {history.map((item, i) => (
                        <div key={item.id} className="flex gap-3 group">
                            ...
                            <div className="flex flex-col items-center shrink-0">
                                <div
                                    ...
                                    style={{
                                        backgroundColor: item.userColor ?? "#6b7280",
                            ...
                            <div className={`flex-1 min-w-0 pb-4 ${i === history.length - 1 ? "pb-2" : ""}`}>
                                <div className={`rounded-xl border p-2.5 transition-all group-hover:shadow-sm ${isDark ? "border-gray-700/50 bg-gray-900/50 group-hover:border-gray-600" : "border-gray-100 bg-white group-hover:border-gray-200 group-hover:shadow-sm"}`}>
                                    <div className="flex items-start justify-between gap-2 mb-1">
                                        <span className={`text-[11px] font-semibold ${isDark ? "text-gray-200" : "text-gray-800"}`}>
                                    ...
                                    <p className={`text-[12px] font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                                        {item.action ?? ""}
                                    </p>
