import { FileText, Clock, Folder, Building2 } from "lucide-react";
import { DropdownMenuItem, DropdownMenuSeparator } from "@/components/ui/dropdown-menu";
import { Edit3, Share2, Download, Trash2, Star } from "lucide-react";
...
export const recentAction = {
  render: (s: RecentSheetRow) => (
    <>
      ...
      <DropdownMenuItem className="text-xs gap-2">
        <Star className="h-3.5 w-3.5" /> {s.isStarred ? "Unstar" : "Star"}
      </DropdownMenuItem>
      <DropdownMenuSeparator />
      <DropdownMenuItem className="text-xs gap-2 text-red-600 focus:text-red-600">
        <Trash2 className="h-3.5 w-3.5" /> Remove from Recent
      </DropdownMenuItem>
    </>
  ),
};
