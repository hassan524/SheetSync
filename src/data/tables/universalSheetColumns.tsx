...
import { FileText, Clock, Building2, Folder, Star, StarOff, Loader2 } from "lucide-react";
import {
  DropdownMenuItem,
...
import { Edit3, Share2, Download, Trash2, FileSpreadsheet, Layers, Code2, Printer } from "lucide-react";
  ...
  render: (s: UniversalSheetRow) => {
    if (s.source === "organization" && s.organizationName) {
      return (
        <Badge
          variant="outline"
          className="text-[11px] gap-1 font-normal py-0 px-2 h-5 bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 max-w-[130px]"
        >
...
// ── Shared action menu ─────────────────────────────────────────────
function SheetActionMenu({ sheet, onDeleted }: { sheet: UniversalSheetRow; onDeleted?: (id: string) => void }) {
  ...
  return (
    <>
      ...
      <DropdownMenuSub>
        <DropdownMenuSubTrigger className="text-xs gap-2" disabled={downloading}>
          {downloading ? (
        ...
        <DropdownMenuSubContent className="w-40">
          <DropdownMenuItem className="text-xs gap-2" onClick={() => handleDownload("csv")}>
            <FileSpreadsheet className="h-3.5 w-3.5 text-green-600" /> CSV (.csv)
          </DropdownMenuItem>
          <DropdownMenuItem className="text-xs gap-2" onClick={() => handleDownload("xlsx")}>
            <Layers className="h-3.5 w-3.5 text-blue-600" /> Excel (.xlsx)
          </DropdownMenuItem>
          <DropdownMenuItem className="text-xs gap-2" onClick={() => handleDownload("pdf")}>
            <Printer className="h-3.5 w-3.5 text-red-500" /> PDF (.pdf)
          </DropdownMenuItem>
          <DropdownMenuItem className="text-xs gap-2" onClick={() => handleDownload("json")}>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent className="max-w-sm">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-base">Delete sheet?</AlertDialogTitle>
            <AlertDialogDescription className="text-sm">
              <span className="font-medium text-foreground">"{sheet.title}"</span> will be permanently deleted. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="h-8 text-xs" disabled={deleting}>Cancel</AlertDialogCancel>
            <AlertDialogAction
