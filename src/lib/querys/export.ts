...
async function fetchSheetForExport(sheetId: string): Promise<SheetExportData> {
  ...
  if (columns.error)
    throw new Error(`Columns load failed: ${columns.error.message}`);
...
function exportXLSX(data: SheetExportData) {
      ...
      if (col.type === "checkbox") {
        ws[ref].s = {
          font: {
            sz: 12,
            color: {
              rgb: rawVal === true || rawVal === "true" ? "059669" : "9CA3AF",
            },
          },
          alignment: { horizontal: "center", vertical: "center" },
          fill: isEvenRow
  ...
  // ── Row heights ──────────────────────────────────────────
  ws["!rows"] = [
    { hpt: 30, hpx: 30 }, // header row taller
    ...rows.map(() => ({ hpt: 28, hpx: 28 })),
  ];
...
function exportPDF(data: SheetExportData) {
  ...
  autoTable(doc, {
    ...
    didDrawPage(pageData) {
      ...
      // Center: sheet title
      doc.text(title, pageW / 2, doc.internal.pageSize.getHeight() - 5, {
        align: "center",
      });
